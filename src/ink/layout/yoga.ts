 ...
 * Yoga 布局适配器
 *
 * 将 yoga-layout 封装为 LayoutNode 接口。
 * 参考 claude-code src/ink/layout/yoga.ts
 */
...
export class YogaLayoutNode implements LayoutNode {
  ...
  /** Auto flex basis (not supported by all yoga-layout versions) */
  setFlexBasisAuto(): void {
    // Not directly supported in yoga-layout v3.2.1 Node.js API
    // This is achieved by setting flexBasis to NaN or not setting it
  }

  setFlexWrap(wrap_: LayoutWrap): void {
    const map: Record<LayoutWrap, Wrap> = {
      nowrap: Wrap.NoWrap,
      wrap: Wrap.Wrap,
      'wrap-reverse': Wrap.WrapReverse,
    };
    this.yoga.setFlexWrap(map[wrap_]!);
  }
