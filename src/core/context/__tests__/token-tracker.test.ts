...
describe('TokenTracker', () => {
  const createUsage = (overrides?: Partial<{ input: number; output: number }>) =>
    ({
      input: overrides?.input ?? 100,
      output: overrides?.output ?? 50,
      totalTokens: (overrides?.input ?? 100) + (overrides?.output ?? 50),
    }) as import('@/core/agent-runtime/runtime-types').Usage;
  ...
  it('getUsage returns correct TokenUsage snapshot', () => {
    const tracker = new TokenTracker();
    tracker.recordUsage(createUsage({ input: 100, output: 50 }));
