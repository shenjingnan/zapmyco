---
title: Zapmyco
---

<p align="center">
  <img src="/logo.svg" alt="zapmyco logo" width="240" lightbox={false} style={{ border: 'none', borderRadius: 0 }} />
</p>

<div style={{ display: 'flex', gap: '4px', flexWrap: 'wrap', justifyContent: 'center' }}>
  <a href="https://github.com/shenjingnan/zapmyco/actions/workflows/ci.yml"><img src="https://github.com/shenjingnan/zapmyco/actions/workflows/ci.yml/badge.svg" alt="CI" lightbox={false} /></a>
  <a href="https://crates.io/crates/zapmyco"><img src="https://img.shields.io/crates/v/zapmyco.svg?color=brightgreen" alt="crates.io" lightbox={false} /></a>
  <a href="https://opensource.org/licenses/MIT"><img src="https://img.shields.io/badge/License-MIT-brightgreen.svg" alt="License: MIT" lightbox={false} /></a>
</div>

**Zapmyco** 是一个基于 Rust 的 AI 驱动命令行工具，提供与 LLM 的交互式聊天体验。

## 特性

- **Rust 实现** — 单二进制文件（~5-10MB），零运行时依赖，毫秒级启动
- **AI 集成** — 内置 AI Agent 对话模式，支持流式输出
- **快速笔记** — 随时随地记录灵感、待办和想法，纯本地 Markdown 存储
- **多平台分发** — crates.io 发布 + 5 平台预编译二进制
- **多供应商** — 内置 DeepSeek、GLM（智谱）模型，支持自定义
- **自动化发布** — GitHub Actions 驱动的自动构建、测试和发布

## 安装方法

### 一键安装（推荐）

<Tabs>

<Tab title="macOS / Linux">
```bash
curl -fsSL https://zapmyco.com/install.sh | sh
```
</Tab>

<Tab title="Windows (PowerShell)">
```powershell
irm https://zapmyco.com/install.ps1 | iex
```
</Tab>

<Tab title="Windows (cmd.exe)">
```powershell
powershell -c "irm https://zapmyco.com/install.ps1 | iex"
```
</Tab>

</Tabs>

### 通过 cargo 安装

需要安装 [Rust 工具链](https://rustup.rs)：

```bash
cargo install zapmyco
```

### 手动下载

每个版本都会发布预编译的二进制归档（`.tar.xz` / `.zip`）：

| 平台    | 架构          | 下载链接 |
| ------- | ------------- | -------- |
| Linux   | x86_64        | [zapmyco-x86_64-unknown-linux-gnu.tar.xz](https://github.com/shenjingnan/zapmyco/releases/latest/download/zapmyco-x86_64-unknown-linux-gnu.tar.xz) |
| Linux   | ARM64         | [zapmyco-aarch64-unknown-linux-gnu.tar.xz](https://github.com/shenjingnan/zapmyco/releases/latest/download/zapmyco-aarch64-unknown-linux-gnu.tar.xz) |
| macOS   | Apple Silicon | [zapmyco-aarch64-apple-darwin.tar.xz](https://github.com/shenjingnan/zapmyco/releases/latest/download/zapmyco-aarch64-apple-darwin.tar.xz) |
| macOS   | Intel         | [zapmyco-x86_64-apple-darwin.tar.xz](https://github.com/shenjingnan/zapmyco/releases/latest/download/zapmyco-x86_64-apple-darwin.tar.xz) |
| Windows | x86_64        | [zapmyco-x86_64-pc-windows-msvc.zip](https://github.com/shenjingnan/zapmyco/releases/latest/download/zapmyco-x86_64-pc-windows-msvc.zip) |

### 从源码编译

```bash
git clone https://github.com/shenjingnan/zapmyco.git
cd zapmyco
cargo build --release
# 产物位于 target/release/zapmyco

# 验证编译结果
./target/release/zapmyco -V

# 安装到 PATH
cargo install --path .

# 验证安装
zapmyco -V
```

## 验证安装

安装完成后，运行以下命令验证：

```bash
zapmyco --version
# 输出示例：zapmyco 0.25.1

zapmyco --help
# 输出示例：
# 基于 Rust 的 AI 驱动命令行工具
#
# Usage: zapmyco [COMMAND]
#
# Commands:
#   note      快速记录笔记
#   settings  显示 LLM 配置
#   run       一次性执行 AI 任务
#   init      初始化 LLM 配置
#   completion 生成 shell 补全脚本
#   help      显示帮助信息
```

## 快速使用

```bash
# 初始化配置
zapmyco init

# 查看帮助
zapmyco --help

# AI 对话模式
zapmyco
```

## 下一步

- [初始化配置](/commands/init) — 运行 `zapmyco init` 配置 API Key
- [命令参考](/commands/overview) — 了解所有命令的详细用法
- [内置工具](/guide/built-in-tools) — 了解 `run` 命令的 AI Agent 工具能力
- [file_edit 实现详解](/guide/file-edit) — 深入理解文件编辑工具的设计原理和安全模型
- [AI 代理功能](/guide/ai-agent) — 体验 AI 对话模式
- [架构说明](/advanced/architecture) — 了解项目设计
