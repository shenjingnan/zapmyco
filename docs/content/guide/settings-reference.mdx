---
title: 配置文件参考
description: ~/.zapmyco/settings.toml 所有字段说明与示例
---

`~/.zapmyco/settings.toml` 是 ZapMyCo 的 LLM 配置文件，用于管理 AI 供应商和模型映射。

> 该文件**完全可选**。如果不存在，ZapMyCo 会回退到 `DEEPSEEK_API_KEY` 环境变量和默认模型 `deepseek-v4-flash`。
