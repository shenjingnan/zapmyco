...
shell_exec 工具自动放行配置。在此配置的命令会跳过用户确认，直接执行。

| 字段 | 类型 | 必填 | 说明 |
|------|------|:----:|------|
| `allow` | 字符串数组 | 否 | 放行命令前缀列表，匹配的命令跳过确认 |

**匹配规则**：命令以列表中的条目开头即匹配（前缀匹配）。
例如 `"git status"` 会放行 `git status`、`git status -s`、`git status --short` 等。
...
> 内置已有绝对安全命令列表（`pwd`、`whoami`、`echo`、`ls`、`uname` 等），无需在此重复配置。
> 此配置仅用于扩展内置列表之外的自定义命令。
