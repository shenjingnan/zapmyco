...
| `file_edit` | 编辑文件内容 | 使用 `old_string`/`new_string` 模式精确替换文件中的文本，比 sed 更安全可靠 |
...
> **sed** 用正则匹配，容易误改；**file_edit** 用精确字符串匹配，更安全。  
> **file_read** 用于查看文件内容；**file_edit** 用于修改文件内容。  
> AI Agent 会自动组合两者：先用 `file_read` 查看文件内容，再用 `file_edit` 进行精确替换。
...
| 版本 | 新增工具 | 说明 |
|------|----------|------|
| 当前 | `web_fetch`, `web_search`, `shell_exec`, `file_read`, `file_search`, `file_edit` | 初始 6 个内置工具 |
| 未来 | *待定* | *敬请期待* |
